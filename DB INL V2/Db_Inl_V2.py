...
def show_bookings():
    ...
    con = connect_database()
    cur = con.cursor()
    cur.execute("""SELECT t.prn_name, t.email, 
                    tr.departure_time, tr.arrival_time, 
                    b.departure_des, b.arrival_des,
                    bs.street_name  
                    FROM travelers t INNER JOIN bookings bo
                    ON t.id = bo.prn_id INNER JOIN trip_info tr
                    ON tr.trip_id = bo.trip_id INNER JOIN bus_info b 
                    ON tr.bus_id = b.id INNER JOIN bus_stop bs 
                    ON tr.bus_id = bs.bus_id
                    WHERE t.email=%s""", (email,))
    all_results = cur.fetchall()
    print("|{:<10}|{:<15}|{:<15}|{:<15}|{:<15}|{:<15}|{:<15}".format("Name", "Email", "Departure time", "Arrival time", "Departure from", "Arrival to", "Arrival address")) 
    for res in all_results:
        print("|{:<10}|{:<15}|{}{:<7}|{}{:<7}|{:<15}|{:<15}|{:<15}".format(res[0], res[1], res[2], "", res[3], "", res[4], res[5], res[6]))
    print("")
    main()
